...
class Player(Collidable):
    ...
    def update(self):
        ...
        if dx > 0:
            self.image = self.right_images[int(self.frame/6)%4]
        if dx < 0:
            self.image = self.left_images[int(self.frame/6)%4]
    ...
    def update(self):
        ...
        if not self.dead:
            ...
            if self.speed < 1:
                self.images = self.left_images
            self.frame += 1
            self.image = self.images[int(self.frame/50)%4]
            if self.hit_timer > 0:
                self.image = self.images[int(self.frame/50)%4 + 1]
